...
var simulation = d3.forceSimulation()
    .force("link", d3.forceLink().id(function (d) {return d.id}).strength(0.08)) //節點之間的距離
...
function setup(links, nodes) {
    ...
    node.append("circle")
        .attr("r", 8) // node 大小
        .style("fill", function (d, i) {
			console.log(JSON.parse(d.total).social);
			if(JSON.parse(d.total).social == "twitter") return "#B088FF";
			if(JSON.parse(d.total).social == "fb") return "#66CDAA";
			if(JSON.parse(d.total).social == "total") return "orangered"})

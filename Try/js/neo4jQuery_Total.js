...
function getLinks(){
    return session.run("match (n:TotalUser)-[r]->(o) return (id(n)), type(r), (id(o)), n.social AS social").then(function (result){
        var links=[]
...
function getNodes(){
    ...
    return session.run("match (n:TotalUser) return properties(n), labels(n), id(n), n.social AS social").then(function (result){
        var nodes=[]
        result.records.forEach(function (rec){

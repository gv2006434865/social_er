...
function getLinks(){
    return session.run("match (n:FbUser)-[r]->(o) return (id(n)), type(r), (id(o))").then(function (result){
        var links=[]
...
function getNodes(){
    ...
    return session.run("match (n:FbUser) return properties(n), labels(n), id(n)").then(function (result){
        var nodes=[]
        result.records.forEach(function (rec){
